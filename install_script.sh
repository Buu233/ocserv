...
function sys_clean(){
	...
	sed -i '/iptables -A INPUT -p tcp --dport 4482 -j ACCEPT/d' /etc/rc.d/rc.local
	sed -i '/iptables -A INPUT -p udp --dport 4482 -j ACCEPT/d' /etc/rc.d/rc.local
...
iptables -A INPUT -p tcp --dport 4482 -j ACCEPT
iptables -A INPUT -p udp --dport 4482 -j ACCEPT
