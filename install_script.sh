...
function sys_clean(){
	...
	sed -i '/iptables -A INPUT -p tcp --dport 2066 -j ACCEPT/d' /etc/rc.d/rc.local
	sed -i '/iptables -A INPUT -p udp --dport 2066 -j ACCEPT/d' /etc/rc.d/rc.local
...
cd /etc/ocserv/
rm -rf ocserv.conf
wget --no-check-certificate https://raw.githubusercontent.com/Buu233/ocserv/7/ocserv.conf
#
cd /root/anyconnect
...
iptables -A INPUT -p tcp --dport 2066 -j ACCEPT
iptables -A INPUT -p udp --dport 2066 -j ACCEPT
